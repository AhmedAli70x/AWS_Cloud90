...
# create public subnet 10.0.2.0/24 in secon az
# create private subnet 10.0.3.0/24 in first az
# create private subnet 10.0.4.0/24 in secon az


create_subnet()
{   
    # $1 subnet number, $2 az, $3 public or private
    check_subnet=$(aws ec2 describe-subnets --region eu-west-2 --filters Name=tag:Name,Values=sub-$3-$1-devops90 | grep -oP '(?<="SubnetId": ")[^"]*')
    if [ "$check_subnet" == "" ]; then

    echo "subnet $1 will be created"

    subnet_result=$(aws ec2 create-subnet \
    --vpc-id $vpc_id --availability-zone eu-west-2$2 \
...
if [ "$check_igw" == "" ]; then
    ...
    igw_id=$(aws ec2 create-internet-gateway --region eu-west-2 \
